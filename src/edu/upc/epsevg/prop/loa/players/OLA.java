...
public class OLA implements IPlayer, IAuto {
    ...
    /**
     * Funció que es crida quan passa el temps fixat indicat al crear la
     * partida.
     */
    @Override
    public void timeout() {
...
//        System.out.println("movim: " + movimiento_def.getFrom() + "" + movimiento_def.getTo());
    ...
    /**
     * Funció que retorna el moviment amb menys valor heurístic de tots els
     * moviments estudiats.
     *
     ...
     * @param color variable que fa refèrencia al color del jugador que mou
     * segon.
    ...
    /**
     *
     * Funció que retorna el moviment amb més valor heurístic de tots els
     * moviments estudiats.
     *
     * @param s estat actual del tauler.
     * @param pprof profunditat a la que estem dins del minimax
    ...
    /**
     * Funció que a partir d'un punt donat, retorna el grup que forma em les
     * seves veïnes.
     *
     * @param s tauler actual.
     * @param from punt central del qual es miren les veïnes.
     * @param color color del punt central.
     * @return retorna el conjunt que forma un punt qualsevol amb les del seu
     * voltant.
     */
    ...
    /**
     * Funció que fa la acció de retornar amb una array el major nombre de
     * fitxes connectades d'un determinat color de fitxes.
     *
     * @param s tauler actual.
    ...
    /**
     * Funció que calcula la heurística a partir del grup més gran d'un
     * determinat color, tenint en compte quins punts es queden d'aquest grup.
     *
    ...
    public int getHeuristicaDef(GameStatus gs, CellType color) {
        ...
        int valorMult = 1;
        if (color == this.color) {
            if (puntosFuera.size() <= grupoMaximo.size()) {
                valorMult = 20;
            } else if (puntosFuera.size() > grupoMaximo.size()) {
                valorMult = -30;
            }
        } else {
            if (puntosFuera.size() < grupoMaximo.size()) {
    ...
    /**
     * Funció que segons la distància a la que es trobi un punt respecte un
     * conjunt de punts determinat, retorna un valor o un altre, quant més aprop
     * més valor.
     *
     * @param gs tauler actual
    ...
    /**
     * Funció que fa la suma ponderada de les dues heurístiques calculades, la
     * de centre y la del grup major.
     *
     * @param s tauler actual
